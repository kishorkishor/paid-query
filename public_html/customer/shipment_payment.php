...
function cartons_has_bd_cols(PDO $pdo): bool {
  /**
   * Determine if the inbound_cartons table has the necessary billing columns.
   *
   * Historically the BD billing relied on bd_total_price and bd_payment_status.
   * With partial payments, the table may now include total_paid/total_due in place
   * of bd_payment_status.  We return true if bd_total_price exists and at least
   * one of (bd_payment_status, total_paid, total_due) is present.
   */
  static $has = null;
  if ($has !== null) return $has;
  try {
    $cols = $pdo->query("SHOW COLUMNS FROM inbound_cartons")->fetchAll(PDO::FETCH_COLUMN);
    $hasPrice = in_array('bd_total_price', $cols, true);
    $hasStatus = in_array('bd_payment_status', $cols, true);
    $hasTotalDue = in_array('total_due', $cols, true);
    $hasTotalPaid = in_array('total_paid', $cols, true);
    $has = $hasPrice && ($hasStatus || $hasTotalDue || $hasTotalPaid);
 ...
 *
 * When total_due/total_paid columns exist, we use them to compute the
 * outstanding amount per carton.  Otherwise we fall back to the old logic
 * of summing bd_total_price for pending bd_payment_status.
 */
/**
 * Sum of BD charges still pending for this order.
 *
...
function carton_bd_due(PDO $pdo, int $orderId): float {
  ...
  try {
    // Check presence of the new partial payment columns
    ...
    if ($hasTotalDue || $hasTotalPaid) {
      ...
      $sql = "
        SELECT COALESCE(SUM($dueExpr),0) AS due
    ...
    // Legacy fallback: sum price where payment_status is pending
    $sql = "
...
/**
 * Sum of BD charges pending for specific cartons in this order.
 *
 * This helper mirrors carton_bd_due() but applies a filter on a list of
 * carton IDs.  It supports the partial payment columns total_due/total_paid.
 * If no IDs are provided it falls back to the whole order.
 *
 * @param PDO   $pdo
 * @param int   $orderId
 * @param array $cartonIds List of carton IDs (integers)
 * @return float
 */
function carton_bd_due_selected(PDO $pdo, int $orderId, array $cartonIds): float {
  if (!cartons_has_bd_cols($pdo)) return 0.0;
  // Normalize IDs
  $ids = array_values(array_filter(array_map('intval', $cartonIds), function($v){ return $v > 0; }));
  if (!$ids) {
    // If no IDs specified, return the full order due
    return carton_bd_due($pdo, $orderId);
  }
  try {
    // Check partial payment columns
    $hasTotalDue  = table_has_col($pdo, 'inbound_cartons', 'total_due');
    $hasTotalPaid = table_has_col($pdo, 'inbound_cartons', 'total_paid');
    if ($hasTotalDue || $hasTotalPaid) {
      $dueExpr = $hasTotalDue
    ...
    // Legacy fallback: sum price for pending status
    $sql = "\n      SELECT COALESCE(SUM(c.bd_total_price),0) AS due\n        FROM inbound_cartons c\n        JOIN inbound_packing_lists p ON p.id = c.packing_list_id\n       WHERE p.order_id = ?\n         AND c.id IN (" . implode(',', $ids) . ")\n         AND COALESCE(c.bd_payment_status,'pending') = 'pending'\n    ";
    $st = $pdo->prepare($sql);
    $st->execute([$orderId]);
    return (float)($st->fetchColumn() ?: 0);
...
try {
  ...
  // For BD (shipping) payments, if specific cartons are submitted (carton_ids[]),
  // compute the due only for those cartons; otherwise compute the full due.
  $bdDue = 0.0;
  if ($isBdFinal) {
    // We'll temporarily set bdDue after we know submittedCartonIds below.  Init to full order due.
    $bdDue = carton_bd_due($pdo, $orderId);
  }
  ...
  // If this is a BD payment (final delivery charges) and specific cartons are selected,
  // recompute the BD due and max payable based on those cartons only.  This ensures
  // the UI reflects the amount owed for the selected cartons rather than the full
  // order.  Only update if we haven't processed POST yet (to avoid overriding running sums).
  if ($isBdFinal && $submittedCartonIds) {
    $bdDue = carton_bd_due_selected($pdo, $orderId, $submittedCartonIds);
    $maxPayable = $bdDue;
  }
